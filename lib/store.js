...
export async function deleteMessage(id) {
  ...
  await unpinMessage(id); // a deleted message can't stay pinned
...
export async function clearMessages() {
  ...
  await savePins([]); // clearing the wall also clears pins
...
export async function getPins() {
  if (useKV) {
    const v = await kv(['GET', PIN_KEY]);
    return v ? JSON.parse(v) : [];
  }
  return globalThis.__pins;
}

async function savePins(arr) {
  if (useKV) await kv(['SET', PIN_KEY, JSON.stringify(arr)]);
  else globalThis.__pins = arr;
}

export async function pinMessage(msg) {
  const pins = await getPins();
  if (pins.find((p) => p.id === msg.id)) return { ok: true, pins };
  if (pins.length >= MAX_PINS) return { ok: false, error: 'max', pins };
  const next = [...pins, { id: msg.id, name: msg.name, table: msg.table, text: msg.text }];
  await savePins(next);
...
export async function unpinMessage(id) {
  ...
  if (next.length !== pins.length) await savePins(next);
  return next;
}

export const storageMode = useKV ? 'kv' : 'memory';

// Diagnosa koneksi KV — dipakai /api/health (tidak membocorkan token).
export async function kvHealth() {
  if (!useKV) {
    return { mode: 'memory', ok: true, note: 'Tidak ada env KV terdeteksi (pakai memori).' };
  }
  let host = '?';
  try { host = new URL(url).host; } catch {}
  try {
    const pong = await kv(['PING']);
    return { mode: 'kv', ok: true, ping: pong, host };
  } catch (e) {
    return { mode: 'kv', ok: false, host, error: String(e && e.message ? e.message : e) };
  }
}
